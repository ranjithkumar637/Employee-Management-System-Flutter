...
import '../models/telecast_model.dart';
...
class ProfileProvider extends ChangeNotifier{
  ...
  Future<ProfileModel> getProfile(BuildContext context) async {
    ...
    Future<ProfileUpdateModel> updateProfile(String name, String dob, String location, String companyName,
        ...
        String streetName, String cityId, String stateId, String groundCityId, String groundStateId, String organizerPinCode, List<String> profilePath, String email) async {
      SharedPreferences preferences = await SharedPreferences.getInstance();
      String? accToken = preferences.getString("access_token");
      print(accToken);
  ...
  Future<ResponseModel> updateGroundDetails(String description, List<String> main, List<String> gallery,
    ...
    //get slides
    Future<List<Slides>> getSlides() async{
      slides = [];
      SharedPreferences preferences = await SharedPreferences.getInstance();
      String? accToken = preferences.getString("access_token");
      try {
        final response = await http.get(
          Uri.parse(AppConstants.getSlides),
          headers: {
            'Content-Type': 'application/json; charset=UTF-8',
            'Authorization': 'Bearer $accToken',
          },
        ...
        var decodedJson = json.decode(response.body);
        print("slides list: $decodedJson");
        if (response.statusCode == 200) {
          slideShowModel = SlideShowModel.fromJson(decodedJson);
          for (var slide in decodedJson['slides']) {
            slides.add(Slides.fromJson(slide));
            notifyListeners();
          }
          notifyListeners();
        } else if(response.statusCode == 500){
        ...
        print('slides - Invalid data format');
      } catch (e) {
        print(e);
      }
      return slides;
    ...
    TelecastModel telecaseModel = TelecastModel();
    Telecast telecast = Telecast();

    //get telecast
    Future<TelecastModel> getTelecast() async{
      SharedPreferences preferences = await SharedPreferences.getInstance();
      String? accToken = preferences.getString("access_token");
      try {
        final response = await http.get(
          Uri.parse(AppConstants.getTelecast),
          headers: {
            'Content-Type': 'application/json; charset=UTF-8',
            'Authorization': 'Bearer $accToken',
          },
        ...
        var decodedJson = json.decode(response.body);
        print("telecast: $decodedJson");
        if (response.statusCode == 200) {
          slideShowModel = SlideShowModel.fromJson(decodedJson);
          telecast = Telecast.fromJson(decodedJson['telecast']);
          notifyListeners();
        } else if(response.statusCode == 500){
          print("Internal server error");
        } else if(response.statusCode == 401){
        ...
        print('telecase - Invalid data format');
      } catch (e) {
        print(e);
      }
      return telecaseModel;
    }
